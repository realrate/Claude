...
4. For each company in the list:
   - Run via Bash from the `Mind Map claude` directory:
     ```
     cd "C:\Users\shaky\Downloads\RealRate claude\RealRate\Mind Map claude" && python gen_mindmap.py <company>
     ```
...
Each company writes to its own subfolder inside `Mind Map claude/`:
